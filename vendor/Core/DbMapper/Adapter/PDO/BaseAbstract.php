 ...
 * @package Spot
 */
abstract class Core_DbMapper_Adapter_PDO_BaseAbstract extends Core_DbMapper_Adapter_AdapterAbstract implements Core_DbMapper_Adapter_AdapterInterface
{
    protected $_database;
        ...
        if(!$this->_connection) {
            if($this->_dsn instanceof PDO) {
                ...
                try {
                    ...
                    throw new Core_DbMapper_Exception($e->getMessage());
                }
            }
        }
    ...
    public function migrateTableCreate($table, array $formattedFields, array $options = array())
    {
        ...
        */
        ...
        // Add query to log
        Core_DbMapper_Log::addQuery($this, $sql);
        ...
        */
        ...
        if ( !empty($columnsSyntax) ) {
            ...
            try {
                ...
                if($e->getCode() == "42S02") {
                    throw new Core_DbMapper_Exception_Datasource_Missing("Table '" . $table . "' does not exist");
                }
    ...
    {
        // Add query to log
        Core_DbMapper_Log::addQuery($this, $sql, $binds);
        ...
        if($stmt = $this->connection()->prepare($sql)) {
            ...
            throw new Core_DbMapper_Exception(__METHOD__ . " Error: Unable to execute SQL query - failed to create prepared statement from given SQL");
        }
    }
    ...
    public function create($datasource, array $data, array $options = array())
    {
        ...
        try {
            ...
            if($e->getCode() == "42S02") {
                throw new Core_DbMapper_Exception_Datasource_Missing("Table or datasource '" . $datasource . "' does not exist");
            }
     ...
     */
    public function read(Core_DbMapper_Query $query, array $options = array())
    {
        $isCount = false;
        ...
        try {
            ...
            // Table does not exist
            if($e->getCode() == "42S02") {
                throw new Core_DbMapper_Exception_Datasource_Missing("Table or datasource '" . $query->datasource . "' does not exist");
            }
     ...
     */
    public function count(Core_DbMapper_Query $query, array $options = array())
    {
        ...
        foreach($conditions as $condition) {
            ...
            foreach($subConditions as $column => $value) {
                ...
                switch($operator) {
                    ...
                    case ':all':
                        throw new Core_DbMapper_Exception("SQL adapters do not currently support the ':all' operator");
                    break;
                    // Not equal
                    case '<>':
                    case '!=':
     ...
     */
    public function toCollection(Core_DbMapper_Query $query, $stmt)
    {
        $mapper = $query->mapper();
