 ...
 */
class Sqlite extends Core_DbMapper_Adapter_PDO_BaseAbstract implements Core_DbMapper_Adapter_AdapterInterface
{
        ...
        if (!$this->_connection) {
            if ($this->_dsn instanceof PDO) {
                ...
                if (preg_match('/(\w+)\.\w+$/i', $this->_dsn, $matches)) {
                    ...
                    throw new Core_DbMapper_Exception('Database not found in Sqlite DSN');
                }
            }
        }
    ...
    public function migrateSyntaxFieldCreate($fieldName, array $fieldInfo)
    {
        ...
        if (!isset($this->_fieldTypeMap[$fieldInfo['type']])) {
            throw new Core_DbMapper_Exception("Field type '" . $fieldInfo['type'] . "' not supported");
        }
