 ...
 *
 */
abstract class Core_DbMapper_Entity {
	...
	protected function initFields() {
		$fields = call_user_func(array(get_class($this), 'fields'));
	...
	public static function datasourceOptions($dsOpts = null) {
		if (null !== $dsOpts) {
			self::$_datasourceOptions = $dsOpts;
			return $this;
		}
	...
	public function data($data = null, $modified = true) {
		...
		if (is_object($data) || is_array($data)) {
			/**
			 * @todo substituir pela chama estática.
			 */
			$fields = call_user_func(array(get_class($this), 'fields'));
			...
			foreach($data as $k => $v) {
				...
				if (array_key_exists($k, $fields)) {
					$typeHandler = Core_DbMapper_Config::typeHandler($fields[$k]['type']);
	 ...
	 * @return self|array|boolean self, getter returns array or boolean if key given and not found
	 */
