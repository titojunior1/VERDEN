 ...
 */
class Core_DbMapper_Query implements Countable, IteratorAggregate, Core_DbMapper_QueryInterface
{
    protected $_mapper;
     ...
     */
    public function __construct(Core_DbMapper_Mapper $mapper, $entityName)
    {
        $this->_mapper = $mapper;
        $this->_entityName = $entityName;
        foreach (self::$_resettable as $field) {
    ...
    public function __call($method, $args)
    {
        ...
        } else if (method_exists('Core_DbMapper_Entity_Collection', $method)) {
...
//         $this->with = array_unique($this->with);
    ...
    public function search($fields, $query, array $options = array())
    {
        ...
        // Only on MySQL
        if($connection instanceof Core_DbMapper_Adapter_Mysql) {
