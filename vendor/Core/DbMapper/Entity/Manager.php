 ...
 * @package Spot
 */
class Core_DbMapper_Entity_Manager {
	...
	public function fields($entityName) {
		if (! is_string($entityName)) {
			throw new Core_DbMapper_Exception(__METHOD__ . " only accepts a string. Given (" . gettype($entityName) . ")");
		}

		// estava sendo usado a função is_subclass_of, mas esta função causa um erro quando existe autoloader configurado
		$obj = new $entityName;

        if(!$obj instanceof Core_DbMapper_Entity) {
            throw new Core_DbMapper_Exception($entityName . " must be subclass of 'Core_DbMapper_Entity'.");
        }

// 		if (! is_subclass_of($entityName, 'Core_DbMapper_Entity')) {
// 			throw new Core_DbMapper_Exception($entityName . " must be subclass of 'Core_DbMapper_Entity'.");
// 		}
		...
		if (isset(self::$_fields[$entityName])) {
			...
			$entityDatasource = call_user_func($entityName . '::datasource');
			if (null === $entityDatasource || ! is_string($entityDatasource)) {
				echo "\n\n" . $entityName . "::datasource() = " . var_export(call_user_func(array($entityName, 'datasource')), true) . "\n\n";
			...
			// Datasource Options
// 			$entityDatasourceOptions = call_user_func(array($entityName, 'datasourceOptions'));
// 			self::$_datasourceOptions[$entityName] = $entityDatasourceOptions;
			self::$_datasourceOptions[$entityName] = array();
			// Connection info
			$entityConnection = call_user_func($entityName . '::connection');
			...
			$entityFields = false;
			$entityFields = call_user_func($entityName. '::fields');
			...
			$entityRelations = call_user_func(array($entityName, 'relations'));
