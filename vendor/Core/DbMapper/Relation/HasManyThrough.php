...
/**
 * Core_DbMapper_Relation_HasManyThrough
 *
 * Classe para o tipo de relacionamento 'has many'
 *
 * @package Core\DbMapper
 */
class Core_DbMapper_Relation_HasManyThrough extends Core_DbMapper_Relation_RelationAbstract implements Countable, IteratorAggregate, ArrayAccess {
	...
	protected function toQuery() {
		...
		if (false !== $twe) {
			...
			$entities = $this->mapper()->all($this->entityName(), $twe)->order($this->relationOrder());
			$entities->snapshot();
			return $entities;
		}
	 ...
	 *
	 * @return \Spot\Entity\Core_DbMapper_Collection
	 */
	public function getIterator() {
		// Load related records for current row
		$data = $this->execute();
		return $data ? $data : new Core_DbMapper_Entity_Collection();
	}
