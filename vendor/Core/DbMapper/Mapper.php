...
class Core_DbMapper_Mapper
{
    ...
    protected $_collectionClass = 'Core_DbMapper_Entity_Collection';
    protected $_queryClass = 'Core_DbMapper_Query';
    protected $_exceptionClass = 'Core_DbMapper_Exception';
     ...
     *  Constructor Method
     */
    public function __construct(Core_DbMapper_Config $config)
    {
        $this->_config = $config;
    ...
    public function entityManager()
    {
        if(null === self::$_entityManager) {
            self::$_entityManager = new Core_DbMapper_Entity_Manager();
        }
     ...
     * @return Core_DbMapper_Adapter_AdapterInterface
     * @throws Exception
     */
...
//         return $this->with($collection, $entityName, $with);
    ...
    public function with($collection, $entityName, $with = array()) {
        ...
        foreach($with as $relationName) {
            ...
            foreach ($collection as $entity) {
                ...
                if ($relationObj instanceof Core_DbMapper_Relation_HasOne) {
                    $relation_collection = array_shift($collectedEntities);
                } else {
                    $relation_collection = new Core_DbMapper_Entity_Collection(
    ...
    {
        if(!is_object($entity)) {
            throw new $this->_exceptionClass("Core_DbMapper_Entity must be an object, type '" . gettype($entity) . "' given");
        }
     ...
     * @return Core_DbMapper_QueryInterface
     */
    ...
    {
        $dumpedData = array();
        $fields = call_user_func(array($entityName, 'fields'));
        foreach($data as $field => $value) {
            $typeHandler = Core_DbMapper_Config::typeHandler($fields[$field]['type']);
    ...
    {
        $entityName = $entity instanceof Core_DbMapper_Entity_Collection? $entity->entityName() : get_class($entity);
        if (!$entityName) {
     ...
     */
    public function validate(Core_DbMapper_Entity $entity)
    {
        $entityName = get_class($entity);

//         $validation = new \Valitron\Validator($entity->data());
...
//                 $validation->rule('in', $field, $fieldAttrs['options']);
            }
...
//                     call_user_func_array(array($validation, 'rule'), $params);
                }
            }
        }
...
//         }
    ...
    public function getHooks($entityName, $hook)
    {
        ...
        if (is_callable(array($entityName, 'hooks'))) {
            $entityHooks = call_user_func($entityName . '::hooks');
