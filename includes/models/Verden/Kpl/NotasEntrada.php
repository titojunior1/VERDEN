...
final class Model_Wms_Kpl_NotasEntrada extends Model_Wms_Kpl_KplWebService {
	...
	function ProcessaNotasEntradaWebservice ( $request ) {
		...
		$this->_climov_id = $app->DbLastInsertId ();
		...
		$qry2 = $app->DbExecute ( $sql );
		if ( $qry2 ) {
			while ( $row = $app->DbFetchAssoc ( $qry2 ) ) {
		...
		if ( count ( $array_pedidos ) > 0 ) {
			...
			foreach ( $array_pedidos as $indice => $dados_pedidos ) {
				...
				$sql = "SELECT forn_id_cli FROM fornecedores WHERE cli_id={$this->_cli_id} and forn_cnpj='{$dados_pedidos['CNPJFornecedor']}'";
				...
				if ( $erros_pedidos == 0 ) {
					...
					// se cliente for abril, adiciona serie na consulta
					$and_note_serie = $this->_cli_id==75? "AND (note_serie = '{$dados_pedidos['SerieNF']}' )": "";
